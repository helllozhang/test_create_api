...
@ddt
class Test_Createcase(unittest.TestCase):
    ast = Asser()
    excel = Do_excel(os.path.join(excel_path,'api_data.xlsx'),'createcase')
    ...
    def setUpClass(cls) -> None:
        ...
        parms1 = {
                "name":CaseDate.name,
                "tester": "小菜鸡",
                "project_id": CaseDate.pid,
                "desc": "嘻嘻嘻嘻"
            }
        headers = {
            "Authorization": getattr(CaseDate, "token")
        }
        res = requests.post(url='http://api.keyou.site:8000/interfaces/',json=parms1,headers=headers).json()
        CaseDate.cid = jsonpath.jsonpath(res,'$.id')[0]
